...
- 설정의 기술 노드·소자 종류·W/L·온도·모델 해시·명목 VDD와 처리 CSV
  메타데이터가 다르거나 VDS/VGS 스윕 조건이 어긋나면 재처리를 요구하도록
  해 오래된 데이터에 새 설정을 잘못 적용하는 경우를 차단했습니다.
- Vth/DIBL 기준값과 민감도 계산이 모두 성공한 뒤에만 결과 파일을 기록해,
  민감도 계산 실패가 기존 결과를 부분 갱신하지 않도록 했습니다.
- 공개 릴리스 검증기가 루트 체크아웃의 `.git`만 제외하고 중첩 `.git`과
  `.venv`/`venv` 파일을 금지하도록 강화했습니다.
- SS 민감도 편차율은 CSV에 공개되는 12자리 유효숫자 값을 기준으로 계산해,
  NumPy/BLAS 구현이 달라도 검증 CSV가 동일하게 재생성되도록 했습니다.
...
- README에 Vth/DIBL 공식, 기준전류 민감도, 재현 명령, 출력 파일과 해석
  한계를 문서화했습니다.
- 38개 자동 테스트와 54개 파일 릴리스 매니페스트 검증을 통과한 상태로
  v1.1 공개 릴리스를 확정했습니다.
